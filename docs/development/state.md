...
> **Last refresh**: 2026-09-15 (post-2.8.0) | **Refresh cadence**: every release (ideally bumped by the release post-hook).
...
| File | Value | Source |
|---|---|---|
| `VERSION` | **2.8.0** | single source of truth |
| `cyrius.cyml [package].version` | `${file:VERSION}` | reads `VERSION` |
| Latest git tag | `2.7.3` (2.8.0 pending tag) | release workflow asserts `VERSION == tag` |
...
| Target | Lines | Bytes |
|---|---|---|
| `dist/majra.cyr` (core) | 4,956 | 168 KB |
| `dist/majra-signed.cyr` | 5,133 | 175 KB |
| `dist/majra-admin.cyr` | 5,181 | 178 KB |
| `dist/majra-backends.cyr` | 7,644 | 266 KB |
| `src/` total | 8,278 lines across 23 files | — |
...
|---|---|---|---|
| Core | `src/main.cyr` (binary self-tests) | 154 | runs as part of `cyrius build` smoke. 150 → 153 at 2.7.3; 154 at 2.8.0 (`test_error` asserts all 20 deprecated `ERR_*` aliases) |
...
|---|---|---|
| **PostgreSQL: cleartext auth, no TLS** | Open — credentials and queries cross the wire in the clear; the connect fails closed on SCRAM rather than downgrading | plan: [roadmap § 2.8 line](roadmap.md) |
...
| **Shared-memory IPC transport** | parked until a consumer hits the syscall-per-message ceiling | plan: [roadmap § 2.8 line](roadmap.md) |
