...
> **Last refresh**: 2026-06-10 (post-2.4.5) | **Refresh cadence**: every release (ideally bumped by the release post-hook).
...
| Latest git tag | `2.4.5` | release workflow asserts `VERSION == tag` |

## Toolchain

| Pin | Value | Source |
|---|---|---|
| Cyrius | **6.1.24** | `cyrius.cyml [package].cyrius` |
...
> resolution. Run `cyrius lib sync` (copies the 94-file version-pinned
> snapshot into `./lib/`) **before** `cyrius deps` (overlays the sigil
...
|---|---|---|---|
| `lib/sigil.cyr` | **3.7.8** (git tag) | `[deps.sigil]` in `cyrius.cyml` → `cyrius deps` | `src/ipc_encrypted.cyr` (AES-256-GCM), `src/signed_envelope.cyr` (Ed25519) |
...
| `lib/agnosys.cyr` | **1.3.2** (transitive via sigil 3.7.8) | resolved by `cyrius deps` | not directly called from `src/` — pulled by the sigil dist concat (now uses slice subscripts → needs `lib/slice.cyr`) |

Lockfile (`cyrius.lock`) now carries SHA-256 over **94** resolved files (the lib-sync snapshot + the sigil/sakshi/agnosys git deps) — was 3 under the cyrius 5.x model. CI's `cyrius deps --verify` enforces match.

The sigil pin is now **latest (3.7.8)**; the cyrius-5.10.x asm-offset SIGILL that pinned it at 2.9.0 is gone under cyrius 6.x. See [`dependency-watch.md`](dependency-watch.md) for the full bisect history.
...
| Soak | `tests/soak/soak_*.cyr` (4 files) | queue 5k ops, pubsub 2k topics, relay dedup+evict, heartbeat 100×20 + auto-evict | on-demand; all 4 ran clean under 6.1.24 at the 2.4.5 bump |
...
| **6.1.24 pin vs published tag** | pin tracks the in-flight cyrius 6.1.24 release; CI's toolchain install needs the 6.1.24 git tag + release tarball published before it goes green | `cyrius.cyml [package].cyrius` |
