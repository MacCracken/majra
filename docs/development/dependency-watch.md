...
| `slice.cyr`      | First-class 16-byte slice helpers (`_slice_idx_get_W`) — required by agnosys 1.3.2's slice subscripts; provided by `cyrius lib sync` | all (transitively via syscalls→agnosys) |
...
- **Transitive agnosys**: sigil 3.7.8 pulls **agnosys 1.3.2** (was 1.0.4 under sigil 2.9.0). 1.3.2 has zero `SYS_OPEN` refs — the dormant aarch64 cross-build blocker is resolved. agnosys 1.3.2 also uses first-class slice subscripts, so its compilation unit needs `lib/slice.cyr` (provided by `cyrius lib sync`).
...
- **Stdlib changes** — `lib/` is gitignored; under cyrius 6.x `cyrius lib sync` copies the version-pinned snapshot (94 files) into `./lib/` and `cyrius deps` overlays the git deps. The `cyrius.lock` file in-tree carries SHA-256 hashes over all 94 resolved files; CI's `cyrius deps --verify` enforces match. **Always build with `--no-deps`** so the build's auto-`deps` doesn't perturb the synced lib.
- **sigil upgrades** — now tracking **latest (3.7.8)**; the cyrius-5.10.x asm-offset constraint that pinned it at 2.9.0 is gone under cyrius 6.x. On a future bump, rerun the full matrix and watch `test_backends` for any new `ud2`-SIGILL (a missing symbol sigil newly reaches into → add the providing `lib/<mod>.cyr` include). The majra-side payoff already banked: AES-NI / SHA-NI / ed25519-NI hardware acceleration for the `signed` + `backends` profiles. QUIC transport is a separate longer-horizon item; needs X25519 from sigil too.
