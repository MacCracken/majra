...
majra v2.5.x has **two declared external first-party dependencies (sigil,
sakshi)**, both reached only by the richer distribution profiles; the core
profile remains stdlib-only. sakshi is a *transitive* requirement (patra +
sigil both reach it) that majra declares explicitly since 2.5.2 to stop
sigil's older transitive pin from downgrading the snapshot copy — see its
section below. (agnosys is no longer in the graph — sigil 3.8.1 internalized
its trust stack.)
...
| `sakshi.cyr`     | Structured tracing (reached transitively by patra + sigil; version pinned by majra's own `[deps.sakshi]` since 2.5.2) | backends |
...
### sakshi = 2.4.6 (latest) — declared since 2.5.2
- **Where**: `[deps.sakshi]` in `cyrius.cyml` (`modules = ["dist/sakshi.cyr"]`), resolved into `lib/sakshi.cyr` by `cyrius deps`. Carried in `cyrius.lock` as the single commit-pin line.
...
- **On the next sigil bump**: re-check whether sigil has moved its own `[deps.sakshi]` forward. If sigil ever pins *ahead* of the snapshot, majra's block becomes the thing holding sakshi back — bump it in the same pass, or drop it if sigil and the snapshot converge.
