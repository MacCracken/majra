...
- **Transitive agnosys**: sigil 3.7.x pulls **agnosys 1.3.2** (was 1.0.4 under sigil 2.9.0; held at 1.3.2 across the 3.7.8 → 3.7.10 bump). 1.3.2 has zero `SYS_OPEN` refs — the dormant aarch64 cross-build blocker is resolved. agnosys 1.3.2 also uses first-class slice subscripts, so its compilation unit needs `lib/slice.cyr` (provided by `cyrius lib sync`).
