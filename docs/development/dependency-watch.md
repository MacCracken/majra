...
majra v2.7.0 has **zero declared git dependencies.** Everything it consumes —
...
| `patra.cyr`      | SQL-backed storage (patra 1.13.10 via cyrius stdlib; full `WHERE` / `ORDER BY` / `LIMIT` / `COUNT(*)` / `MAX()` surface — `src/patra_queue.cyr` retired its 1.1.1-shaped client-side workarounds in 2.4.3) | backends (patra_queue) |
