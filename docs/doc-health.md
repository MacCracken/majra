...
> **Last refresh**: 2026-07-28 (2.5.3 — first `src/` logic change in the 2.5 line: two `fl_alloc`-rooted concurrency races, head-of-line blocking, and wildcard level-alignment. `cyrius-quirks.md` gained **§8** on the `fl_alloc`-vs-`alloc` thread-safety asymmetry, which is the root cause of both races and is load-bearing given CLAUDE.md's "fl_alloc for structs" rule. CHANGELOG + state.md + roadmap.md touched; state.md's build-footprint and test-count tables re-measured, and its "bundle bodies byte-identical" note retired. A behavior change ships in a PATCH — pattern matching — and is flagged in the CHANGELOG against `semver.md`. Prior entry:) | **Prior refresh**: 2026-07-28 (2.5.2 toolchain refresh — cyrius pin 6.4.62 → 6.4.83, sigil 3.11.1 → 3.12.1, sakshi 2.4.3 → 2.4.6 promoted from an inherited transitive pin to a declared `[deps.sakshi]` block. CHANGELOG + state.md + dependency-watch.md + roadmap.md + README.md + CLAUDE.md + threat-model.md touched. threat-model.md carried two pre-existing staleness bugs closed this pass: its Crypto-trust-boundary line still cited the **sigil 3.7.8** pin (four bumps behind) and its Supply Chain section still claimed "one first-party dep". dependency-watch.md gained a sakshi section + a snapshot-shadowing warning in the upgrade considerations. Fixed `benches/bench_all.bcyr` missing the `async`/`dynlib`/`fdlopen` toolchain includes — `cyrius bench`/`cyrius audit` had been failing to compile it since before 2.5.1. No source-logic change; bundle bodies byte-identical to 2.5.0.) | **Prior refresh**: 2026-07-13 (2.5.1 toolchain refresh — cyrius pin 6.3.15 → 6.4.62, sigil 3.9.8 → 3.11.1, agnosys dropped from the graph; `lib sync --full` now load-bearing (99-file snapshot, lockfile 99 hashes). This pass also reconciled the 2.4.7 → 2.5.x drift that 2.5.0 left in state.md / dependency-watch.md / roadmap.md — all three were still 2.4.7-dated. CHANGELOG + state.md + dependency-watch.md + roadmap.md + README.md + CLAUDE.md touched; sigil-footprint review recorded in dependency-watch.md; `cyrius-quirks.md` §4 corrected (local `var[N]` = N bytes vs global `var[N]` = N×8 bytes — proven empirically after an adversarial review flagged a false-positive "overflow" in correctly-sized global buffers). No source-logic change; bundle bodies byte-identical to 2.5.0.) | **Refresh cadence**: when docs are touched, update the affected row. No release attachment.
...
|---|---|---|---|
| `README.md` | 2026-07-28 | ✅ Fresh | Rust→Cyrius comparison table moved to `cyrius 6.4.83` and both "requires sigil ≥" lines to `3.12.1` at the 2.5.2 bump. Module map + 4-profile distribution surface + consumer list still match. |
| `CHANGELOG.md` | 2026-07-28 | ✅ Fresh | Source of truth for shipped work. Entries through 2.5.2 (cyrius 6.4.62 → 6.4.83, sigil 3.11.1 → 3.12.1, sakshi 2.4.3 → 2.4.6 now explicitly declared, `cyrius bench` entry-point repair, lockfile holds 99 + 1 commit-pin). |
| `CLAUDE.md` | 2026-07-28 | ✅ Fresh | 2.5.2: quick-start snapshot reference moved 6.4.62 → 6.4.83 (the only version-tied string in the file — everything else is durable rules, per design). Rewritten 2026-05-10 against [`first-party-documentation § CLAUDE.md`](https://github.com/MacCracken/agnosticos/blob/main/docs/development/planning/first-party-documentation.md#claudemd) — durable rules only. Volatile state moved to [`docs/development/state.md`](development/state.md); compiler quirks moved to [`docs/development/cyrius-quirks.md`](development/cyrius-quirks.md). Now has all 8 required sections (identity / goal / state-pointer / quick-start / principles / rules / process-with-P(-1) / docs-pointer). ~155 lines, down from 243. |
