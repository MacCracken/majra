...
| `CHANGELOG.md` | 2026-06-15 | ✅ Fresh | Source of truth for shipped work. Entries through 2.4.7 (cyrius 6.1.35 → 6.2.11, sigil 3.7.10 → 3.7.14, agnosys 1.3.2 → 1.4.3, lockfile 88 → 97). |
...
| `VERSION` | 2026-06-15 | ✅ Fresh | `2.4.7` — single source of truth, read into `cyrius.cyml` via `${file:VERSION}`. |
...
| File | Last touched | Status | Notes |
|---|---|---|---|
| `state.md` | 2026-06-15 | ✅ Fresh | Refreshed for the 2.4.7 toolchain bump: version 2.4.7, cyrius pin 6.2.11, sigil 3.7.14, agnosys 1.4.3, 97-entry lockfile (6.2.x snapshot grew 88 → 97 files). Cadence: refresh every release; ideally bumped by the release post-hook. Pair-document with `doc-health.md` (state.md = code state; doc-health.md = doc state). |
...
| `roadmap.md` | 2026-06-15 | ✅ Fresh | 2.4.7 entry added at top of "Recently shipped" (cyrius 6.1.35 → 6.2.11, sigil 3.7.10 → 3.7.14, agnosys 1.3.2 → 1.4.3, lockfile 88 → 97). 2.4.6 entry retained. "Waiting on upstream" empty — both long-standing blockers cleared at 2.4.5. |
| `dependency-watch.md` | 2026-06-15 | ✅ Fresh | Refreshed at the 2.4.7 toolchain bump: sigil heading 3.7.10 → 3.7.14 + 3.7.10→3.7.14 bump bullet (no majra-side change; agnosys rolled 1.3.2 → 1.4.3); "Stdlib changes" consideration now records the 88 → 97 snapshot growth under 6.2.11. Prior 2.4.6 / 2026-05-10 sigil-bisect content carries through. |
